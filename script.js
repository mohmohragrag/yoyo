...
// Links to product images on MediaFire
const imageLinks = {
  bag1: 'https://www.mediafire.com/view/owder2b3coa7698/bag1.jpg/file',
  bag2: 'https://www.mediafire.com/view/dzx7alf9ym10lc0/bag2.jpg/file',
...
function addToCart(name, price, selectedColor) {
  // Get the image link based on the selected product
  let imageLink = imageLinks[name.toLowerCase()] || ''; // Default to empty string if no image found
  
  cart.push({ name, price, color: selectedColor, imageLink });
  updateCart();
  closeProductDetails();
}
...
function updateCart() {
  ...
  cart.forEach(item => {
    ...
    li.textContent = `${item.name} - ${item.price} EGP - Color: ${item.color} - Link: ${item.imageLink}`;
    cartItems.appendChild(li);
  });
}

// Checkout and send order via WhatsApp with MediaFire image links
function checkout() {
  if (cart.length === 0) {
  ...
  let cartSummary = cart.map(item => {
    return `${item.name} - ${item.price} EGP - Color: ${item.color} - Link: ${item.imageLink}`;
  }).join('\n');

  const message = `Hello! I would like to purchase the following products:\n\n${cartSummary}\nThank you!`;

  const whatsappLink = `https://wa.me/+201016975219?text=${encodeURIComponent(message)}`;
